...
INSERT INTO aggregator.headlines (newsfeed, source, url, metadata, https, discussion, labels, fts, thumbnail) (SELECT newsfeed, source, url, metadata, url ~ '^https:', discussion, labels, setweight(to_tsvector(coalesce(metadata->'title','')), 'A') || setweight(to_tsvector(coalesce(metadata->'description', '')), 'B') || setweight(to_tsvector(coalesce(content, '')), 'D'), thumbnail FROM _import) ON CONFLICT DO NOTHING;
