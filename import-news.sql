...
\copy _import (feed, source, url, title, description, content_type, locale, discussion, labels, content, image_url, remote_id) FROM PSTDIN WITH CSV;

INSERT INTO aggregator.headlines (feed, source, url, title, description, content_type, locale, discussion, labels, content, fts, image_url) (SELECT feed, source, url, title, description, content_type, locale, discussion, labels, content, setweight(to_tsvector(coalesce(title,'')), 'A') || setweight(to_tsvector(coalesce(description, '')), 'B') || setweight(to_tsvector(coalesce(content, '')), 'D'), image_url FROM _import) ON CONFLICT DO NOTHING;
