...
INSERT INTO aggregator.headlines (newsfeed, source, url, metadata, https, discussion, labels, fts, thumbnail_url, content) (SELECT newsfeed, source, regexp_replace(url, 'https?:', '', 'i'), metadata, url ~ '^https:', discussion, labels, setweight(to_tsvector(coalesce((metadata->'title')::text,'')), 'A') || setweight(to_tsvector(coalesce((metadata->'description')::text, '')), 'B') || setweight(to_tsvector(coalesce(content, '')), 'D'), thumbnail_url, content FROM _import) ON CONFLICT DO NOTHING;
